...
! The following code segment copies rectangular block of data "buff"
! (which is just a subdomain) into into array "bfr_out" which covers
! the entire physical grid, or vice versa, extracts "buff" from the
! whole-grid array "bfr_in". 

! Arrays "buff","bfr_in", "bfr_out" may be of different type, integer
! of kind=1(byte), 2, or 4, real(kind=4 or 8), or character(len=1), so
! they are declared here, but rather in "partit.F" or "ncjoin.F" where
! this file is used to create different version of the subroutines for
...
!
!          except that formally speaking "buff" is declared as 1D-array
!          because the actual number of its dimensions "vdims" may be
